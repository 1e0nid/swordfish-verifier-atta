...
Судя по названию и коду `run.sh` (`=== Запуск верификатора API Swordfish ===`), проект проверяет, что присланный эксплойт/атака действительно срабатывает против сервиса Swordfish — вероятно, в рамках инфраструктуры Attack-Defense CTF (отсюда `-atta` в названии) или аналогичного соревнования/полигона.

> ⚠️ В репозитории пока нет описания и README пуст, а содержимое `src/`, `config/` и `tests/` недоступно для автоматического анализа (GitHub блокирует просмотр дерева каталогов ботами). Поэтому ниже — черновик на основе того, что реально видно из `requirements.txt`, `run.sh` и `pytest.ini`. Поправь разделы, где я ошибся или упростил, и пришли содержимое `src/main.py`/`config/`, если хочешь более точное описание.

## Возможности (по составу зависимостей)

Судя по `requirements.txt`, проект совмещает веб-API и низкоуровневый анализ бинарей:
